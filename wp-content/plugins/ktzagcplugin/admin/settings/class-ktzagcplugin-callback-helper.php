...
class Ktzagcplugin_Callback_Helper {
	...
	private function get_id_attribute( $id ) {
		return ' id="' . $this->snake_cased_ktzagcplugin . '_settings[' . $id . ']" ';
	}

	private function get_name_attribute( $name ) {
		return ' name="' . $this->snake_cased_ktzagcplugin . '_settings[' . $name . ']" ';
	}
	...
	private function get_label_for( $id, $desc ) {
		return '<label for="' . $this->snake_cased_ktzagcplugin . '_settings[' . $id . ']"> '  . $desc . '</label>';
	}
	...
	public function upload_callback( $args ) {
		...
		$html .= $this->get_id_and_name_attrubutes( $args['id'] );
		$html .= 'class="' . $args['size'] . '-text ' . $this->snake_cased_ktzagcplugin . '_upload_field" ';
		$html .= ' value="' . esc_attr( stripslashes( $value ) ) . '"/>';

		$html .= '<span>&nbsp;<input type="button" class="' . $this->snake_cased_ktzagcplugin . '_settings_upload_button button-secondary" value="' . __( 'Upload File', $this->ktzagcplugin ) . '"/></span>';
