...
class Ktzagcplugin_Sanitization_Helper {
	 ...
	 * @return string
	 */
	public function get_snake_cased_ktzagcplugin() {
		return $this->snake_cased_ktzagcplugin;
	}
	 ...
	 *
	 * Filters in order:
	 * - <snake_cased_ktzagcplugin>_settings_sanitize_<tab_slug>
	 * - <snake_cased_ktzagcplugin>_settings_sanitize_<type>
	 * - <snake_cased_ktzagcplugin>_settings_sanitize
	 * - <snake_cased_ktzagcplugin>_settings_on_change_<tab_slug>
	 * - <snake_cased_ktzagcplugin>_settings_on_change_<field_key>
	 *
	 * @since 	1.0.0
	...
	public function settings_sanitize( $input = array() ) {
		...
		// Tab filter
		$input = apply_filters( $this->snake_cased_ktzagcplugin . '_settings_sanitize_' . $tab, $input );
		...
		add_settings_error( $this->ktzagcplugin . '-notices', $ktzagcplugin, __( 'Settings updated.', $this->ktzagcplugin ), 'updated' );
	...
	private function apply_type_filter( $input, $tab, $key ) {
		...
		return apply_filters( $this->snake_cased_ktzagcplugin . '_settings_sanitize_' . $type, $input[$key], $key );
	}

	private function apply_general_filter( $input, $key ) {

		return apply_filters( $this->snake_cased_ktzagcplugin . '_settings_sanitize', $input[$key], $key );
	}

	// Key specific on change hook
	private function do_settings_on_key_change_hook( $key, $new_value ) {

		$old_plugin_settings = get_option( $this->snake_cased_ktzagcplugin . '_settings' );
		if (isset($old_plugin_settings[$key])) {
			if ( $old_plugin_settings[$key] !== $new_value ) {

				do_action( $this->snake_cased_ktzagcplugin . '_settings_on_change_' . $key, $new_value, $old_plugin_settings[$key] );
	...
	private function do_settings_on_change_hook( $new_values, $tab ) {
		...
		if ( $changed ) {
			...
			do_action( $this->snake_cased_ktzagcplugin . '_settings_on_change_' . $tab, $new_values, $old_plugin_settings );
	...
	private function get_output( $tab, $input ) {
		...
		foreach ( $this->registered_settings[$tab] as $key => $_value ) {
			...
			if ( ! isset( $input[$key] ) ) {
				...
				if ( is_array( $old_plugin_settings[$key] ) ) {
