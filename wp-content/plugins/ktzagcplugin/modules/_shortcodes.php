...
function ktzagcplugin_image_shortcode( $atts ) {
	...
	$relateds = empty($related) ? false : $related;
	if ( $sources == 'ask' ) {
		return ktzplg_get_ask_image( $keywords, $numbers, $relateds );
	} else {
		return ktzplg_get_bing_image( $keywords, $numbers, $relateds );
	}
}
add_shortcode('ktzagcplugin_image', 'ktzagcplugin_image_shortcode'); /* format [ktzagcplugin_image keyword="" source="" number=""] */
